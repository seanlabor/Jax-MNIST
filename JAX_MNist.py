# -*- coding: utf-8 -*-
"""Kopie von Kopie von Kopie von Tutorial 3: JAX - Building a Neural Network from Scratch.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1V74Wovw8usyOxKFPYOHp8Y0GSYlcF9k9
...
@jit
def train(key,conv_weights, imgs, lbls,MLP_params ):
  ...
  # Create a MLP
          ...
          print(type(imgs))
          print("anfang",jnp.shape(imgs)) 
          imgs=jnp.moveaxis(imgs, 1, 3)
          print("np.moveaxis",jnp.shape(imgs)) 
          imgs = conv_apply(conv_weights, imgs)
          print("conv_apply",jnp.shape(imgs))
          
          imgs = jnp.reshape(imgs,(-1,2500)) # Flatten
          print("jnp.reshape",jnp.shape(imgs))
...
for imgs, lbls in train_loader: 
  print(type(imgs))
  print("anfang",np.shape(imgs)) 
  imgs=np.moveaxis(imgs, 1, 3)
  print("np.moveaxis",np.shape(imgs)) 
  imgs = conv_apply(father_weights, imgs)
  print("conv_apply",np.shape(imgs))
  imgs = jnp.reshape(imgs,(-1,2500)) # Flatten
  jax.numpy.reshape(a, newshape, order='C')
  print("imgs.reshape",np.shape(imgs))
  print("=====")
